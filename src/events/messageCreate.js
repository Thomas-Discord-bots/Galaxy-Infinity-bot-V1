...
async function handleCountingGame(message, client) {
  try {
    ...
    // Een gebruiker mag niet twee correcte nummers achter elkaar plaatsen.
    const invalidAttempt = !validCount || message.author.id === config.lastUserId;

    if (invalidAttempt) {
      // Alleen het foute bericht verwijderen.
      // De streak, het volgende nummer en de leaderboard blijven behouden.
    ...
    // - lastUserId bijwerken
    await recordCorrectCount(client, message.guild.id, message.author.id);

    // Geef een vinkje op een correct bericht.
